...
// async function fetchStations(url, cb) {
...
// rewrite fetchStations to use node-fetch
// async function fetchStations(url, cb) {
const fetchStations = async (url) => {
...
// log the stations array to the console
// fetchStations(csv_url, (stations) => {
//   console.log(stations);
// });

async function fetchData(url, cb) {
  const stations = await fetchStations(url);
  //   console.log(stations);
  let foo = await stations.slice(0, 10);
  ...
  parallelLimit(requests, 16, (err, results) => {
...
// generateRequests().then((data) => console.log(data));
